  ...
  return (
    ...
    >
                ...
                {filterOptions.map((option) => (
                  <button
                    ...
                    className={`w-full px-4 py-2 text-left text-theme-sm transition-colors hover:bg-gray-50 dark:hover:bg-gray-700 ${
                      activeFilter === option ? 'text-brand-500 font-bold bg-brand-50/20 dark:bg-brand-500/10' : 'text-gray-600 dark:text-gray-400'
                    }`}
                  >
                    {option}
                  </button>
      ...
      <div className="flex items-center gap-1 ml-auto" style={{ WebkitAppRegion: 'no-drag' }}>
        ...
        {onAction && (
          <button onClick={onAction} className="p-0.5 text-gray-900 dark:text-gray-100 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors focus:outline-none">
            <svg className="size-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
